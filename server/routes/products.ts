...
// Get all products that match prefix
router.get('/products/search/:prefix?', async (req: Request, res: Response): Promise<void> => {
	try {
		const { prefix } = req.params

		let products
		if (prefix === "" || prefix === undefined) {
			products = await productsModel.find() // Return all
		}
		else {
			// $options: 'i' means ignore cases
			products = await productsModel.find({
				product_name: { $regex: `^${prefix}`, $options: 'i' }
			})
		}
		...
		res.status(500).json({ errorMessage: "Search products error: ", error })
		console.log(error)
		return
	}
})
